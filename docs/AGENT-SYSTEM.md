...
"agents" — the lifecycle records that wrap an agent CLI (Claude Code, Codex,
...
only its kind + last status are recorded on the agent record.

> Naming note: "agents" were called "tasks" until ADR-166 renamed the concept
> across the codebase, UI, IPC channels, HTTP routes, and MCP tools.
...
### `AgentInfo` (`electron/agent-persistence.ts:7-26`)
...
calls `agents:setPaneContext` (`ipc/agents.ts:62-75`) to register
`(projectId, projectName, workspacePath, agentCommand)` for the pane.

> **🚩 Flaw — `paneContextMap` is never cleaned up.** Set on
> `agents:setPaneContext`, never deleted. There is no `agents:clearPaneContext`
...
> spurious Notification flips the agent to `requires_input` indefinitely
...
│       upsert agent; broadcast; maybe notify                           │
...
The "upsert agent" branch (`hook-relay.ts:200-249`) is where new agents come
...
> The renderer then has two agent records both pointing to the same pane.

> **🚩 Flaw — `cwd` is never refreshed.** Set once at agent creation from
...
- `Stop` → `applyStopForSession` → agent stays `status: "active"`,
  `lastAgentStatus: "responded"`. Pane and session stay alive.
- `SessionEnd` → agent becomes `status: "completed"`. State is cleaned up.

This is the right model — an agent is "complete" only when the agent process
...
> applying the pending Stop. The agent is force-completed/errored, which is
> fine, but no subagent-cleanup notification is emitted and the
> `unseenRespondedAgents` set never sees this agent. Could miss notifications
...
> will never be touched. Surface bug: app shows an agent as awaiting input
...
> for an hour and wakes, every active agent's `idle` is suddenly > 60s and
...
`AgentsList.tsx` shows two sections:

- **Active**: `agents.filter(t => t.status === "active")`.
- **Recent**: completed/error agents whose `paneId` is still present in the
...
`status` + `seenAgentIds`. Click → `navigateToAgent(agent)`.

### 6.2 History modal

`AgentsView/AgentsView.tsx` shows everything, grouped Today / Yesterday / This
Week / This Month / Older, filtered by status pill (All / Active /
Completed-incl-error+abandoned). Resume → navigate; delete → IPC
`agents:delete`.

> **✅ Resolved (ADR-136 T2) — pagination.** Boot now calls `agents:getActive` for the sidebar plus `agents:getAll({limit:100, offset:0})` for the first history page; `loadMoreAgents` is wired to a scroll sentinel in the history modal.

> **✅ Resolved (ADR-136 T2) — retention.** `AgentManager` constructor calls `pruneOlderThan(agentRetentionDays)` (default 90, 0 disables). One-time toast surfaces the prune count via `agents:consumePruneNotice`, gated by the `agentPruneNoticeShown` preference.
...
> **✅ Resolved (ADR-136 T4) — atomic navigation.** Added `navigateToContext` to `useAppStore`: a single `set(state => ...)` call that updates `activeWorkspacePath`, `activePanelId`, `selectedTabId`, and `focusedPaneId` together. `navigateToAgent` calls it once instead of four discrete `setState`s.

### 6.4 Toast / notification dual-source

`receiveAgentUpdate` (`store/agent-store.ts:108-222`):

- Compares `prevStatus` vs `nextStatus` to decide whether to show a toast.
- Skips toast if the agent's pane is already visible in the active tab.
- Calls `markSeen` (renderer + main) when visible.

The OS-level desktop notification is fired separately by `maybeSendNotification`
in the **main process** based on its own preferences and the `agent-updated`
broadcast. The two paths are independent.

> **✅ Resolved (ADR-136 T3) — single source of truth for unseen.** Main is authoritative; renderer holds a cache. `agent-updated` broadcast payload is now `(agent, { responded, requires_input })` — flags reflect main's Sets at broadcast time. Renderer drops its local "clear seen on status change" branch. `agents:markSeen` re-broadcasts via `sendAgentUpdate` so the cache stays in sync. Boot primes the cache from `agents:getUnseen`. Single send-site: `sendAgentUpdate` in `electron/notifications.ts`.
...
> **✅ Resolved — IPC trust boundary.** Renderer can no longer rewrite `agentSessionId` (the Map key) or any lifecycle field. `agents:markResumed` provides the one auxiliary path the renderer needed.

### 7.3 `agents:get` is O(n)

`ipc/agents.ts:32-36` does `agentManager.getAllAgents().find(t => t.id === agentId)`
on every call. With 10k agents plus the sort step inside `getAllAgents`, this
is gratuitous. `AgentManager` only indexes by `agentSessionId`, not by `id`.

### 7.4 `agent-updated` broadcast (main → renderer)

Single channel, payload is `(agent: AgentInfo, unseen: { responded: boolean; requires_input: boolean })` (ADR-136 T3). The unseen flags reflect main's `unseenRespondedAgents` / `unseenInputAgents` Sets at the moment of the broadcast. Sent via `sendAgentUpdate` in `electron/notifications.ts` — the single send-site, called from the relay, the stream handler (when the agent title changes), `agents:abandonForPane`, `agents:reconcileStale`, and `agents:markSeen`. No batching, no diff.
...
`saveState()` (`agent-persistence.ts:66-77`): debounced 500ms. On every
...
> atomic. Crash mid-write yields a truncated/half-written `agents.json` and
> `loadState` returns an empty Map (the catch-all at line 61 swallows the
> JSON parse error). Result: **all agent history lost** on a crash during
> the 500ms debounce window. Should write to `agents.json.tmp` then
> `fs.renameSync`.
...
first. The relay does have logic to migrate `paneId` on the existing agent
(`hook-relay.ts:206-210`) — but it does so by **clearing the old pane**, so
both panes silently end up sharing one agent while one of them no longer has
any agent at all.

### 8.3 Reconciliation

On main-process startup, `agents:reconcileStale` (`ipc/agents.ts:104-144`)
diffs in-memory active agents against the daemon's live session list. It
abandons any active agent whose `agentSessionId` is not in the live set
...
> without booting, abandoned agents linger as "active" until the orphan
> sweep catches them 60s later — and only if their lastStatus was
> thinking/working.

> **🚩 Flaw — `agentSessionId` ≠ daemon `sessionId`.** The reconcile compares
> `agent.agentSessionId` (the agent CLI's session id, e.g. Claude's UUID)
...
3. Subagent tracking (`activeSubagents` Set) defers `Stop` so the parent
   agent doesn't flip to `responded` while a subagent is running.
...
- Pane close → renderer's `closePaneById` → `agents.abandonForPane(paneId)`.
- Pane stays in layout until manually closed; recent completed agents remain
  visible in the sidebar's "Recent" section as long as the pane exists.

### 10.2 Project / workspace

- Linked at agent creation time via `paneContextMap`.
- Never updated. If a user moves a workspace path on disk, all existing
  agents with the old path are unfindable by `agents:getAll({projectId})`
  going forward.
...
1. **`agentSessionId` ≠ daemon sessionId mismatch in `reconcileStale`** — likely false-abandons most active agents after a multi-session pane. Verify against runtime behavior.
2. **Non-atomic `agents.json` write** — crash mid-write silently empties agent history.
3. **Sweeps and `notifyAgentDetectorGone` ignore `requires_input`** — input-pending agents whose process dies become permanent zombies.
4. **`Stop` / `SessionEnd` race on `pendingStopAt`** — pending stop is dropped if SessionEnd arrives, no `unseenRespondedAgents` entry, missed notification.
...
8. ~~**Renderer-trusted `agents:update`**~~ — ✅ Resolved (ADR-136 T1).
...
12. **Orphan-prior-pane-agent on new SessionStart** — old agent's pane is unlinked but status is left as-is.
13. **`agentKind` defaults to `"claude"`** when hook URL omits it; can mislabel codex/pi agents.
...
17. **`agents:get` linear scan + sort on every call.**
18. ~~**Dual seen-flag bookkeeping**~~ — ✅ Resolved (ADR-136 T3: main is authoritative; renderer is a cache).
19. ~~**Multi-step `navigateToAgent` Zustand mutations**~~ — ✅ Resolved (ADR-136 T4: atomic `navigateToContext`).

### Low / speculative

20. **`Notification` matcher trust** — generic notifications may flip an agent to `requires_input` spuriously.
...
2. ✅ Wrap `agents.json` writes in tmp+rename — resolved by ADR-134 (atomic writes).
