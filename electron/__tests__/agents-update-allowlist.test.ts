...
describe("agents.update allowlist", () => {
  ...
  it("throws when namePinned is not a boolean", () => {
    expect(() => agentsUpdate(localCtx(deps as never), "agent-1", { namePinned: "yes" })).toThrow(
  ...
  it("throws when name is not a string or null", () => {
    expect(() => agentsUpdate(localCtx(deps as never), "agent-1", { name: 42 })).toThrow(
  ...
  it("throws when updates contains status field", () => {
    expect(() => agentsUpdate(localCtx(deps as never), "agent-1", { status: "abandoned" })).toThrow(
  ...
  it("throws when updates contains both name and a forbidden field", () => {
    expect(() => agentsUpdate(localCtx(deps as never), "agent-1", { name: "x", status: "active" })).toThrow(
      'agents:update: field "status" is not writable from renderer',
    );
  ...
  it("throws when updates is not an object (string)", () => {
    expect(() => agentsUpdate(localCtx(deps as never), "agent-1", "not-an-object")).toThrow(
  ...
  it("throws when updates is null", () => {
    expect(() => agentsUpdate(localCtx(deps as never), "agent-1", null)).toThrow(
      "agents:update: updates must be an object",
    );
  ...
  it("throws when updates contains agentSessionId", () => {
    expect(() => agentsUpdate(localCtx(deps as never), "agent-1", { agentSessionId: "some-id" })).toThrow(
      'agents:update: field "agentSessionId" is not writable from renderer',
    );
  });

  it("throws when updates contains paneId", () => {
    expect(() => agentsUpdate(localCtx(deps as never), "agent-1", { paneId: "pane-1" })).toThrow(
