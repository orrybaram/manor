...
pub struct PortScannerState {
    pub worktree_paths: Mutex<Vec<String>>,
...
pub fn start_port_scanner(app: AppHandle, state: State<'_, PortScannerState>) {
    ...
    *state.stop_tx.lock().unwrap() = Some(stop_tx);
    ...
    let worktree_paths = state.worktree_paths.lock().unwrap().clone();
    ...
    std::thread::spawn(move || {
        ...
        loop {
            ...
            let ports = scan_ports(&worktree_paths);
            if ports != last_ports {
...
#[tauri::command]
pub fn update_worktree_paths(state: State<'_, PortScannerState>, paths: Vec<String>) {
    *state.worktree_paths.lock().unwrap() = paths;
}

#[tauri::command]
pub fn scan_ports_now(state: State<'_, PortScannerState>) -> Vec<ActivePort> {
    let paths = state.worktree_paths.lock().unwrap().clone();
    scan_ports(&paths)
}

fn scan_ports(worktree_paths: &[String]) -> Vec<ActivePort> {
    ...
    // Resolve worktree paths via CWD lookup
    if !worktree_paths.is_empty() && !results.is_empty() {
        ...
        for port in &mut results {
            if let Some(cwd) = cwds.get(&port.pid) {
                // Find best matching worktree (longest prefix match)
                let best = worktree_paths
                    .iter()
                    .filter(|wt| cwd.starts_with(wt.as_str()))
                    .max_by_key(|wt| wt.len());
                if let Some(best) = best {
                    if *best != home {
                        port.worktree_path = Some(best.clone());
                    }
                }
            }
        }
    ...
    // Only keep ports associated with a worktree
    results.retain(|p| p.worktree_path.is_some());
