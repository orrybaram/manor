 ...
 * desktop store (`src/store/app-store.ts`) and the structural routes
 * (`electron/routes/panes-structural.ts`), which bypass a renderer entirely
 * (ADR-179 D5) — so it lives here instead of being duplicated in both.
 */
